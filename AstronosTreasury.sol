...
contract AstronosTreasury is AstronosAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    IASTRO public immutable ASTRO;
    IsASTRO public sASTRO;
    ...
    /**
     * @notice allow approved address to deposit an asset for ASTRO
    ...
    function deposit(
        ...
        // mint ASTRO needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        ASTRO.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn ASTRO for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        ASTRO.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new ASTRO using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        ASTRO.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or ASTRO from the treasury, using sASTRO as collateral. This might allow an
     * sASTRO holder to provide ASTRO liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(ASTRO)) {
            require(permissions[STATUS.ASTRODEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sASTRO.changeDebt(value, msg.sender, true);
        require(sASTRO.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(ASTRO)) {
            ASTRO.mint(msg.sender, value);
            astroDebt = astroDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sASTRO.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with ASTRO
     * @param _amount uint256
     */
    function repayDebtWithASTRO(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.ASTRODEBTOR][msg.sender], notApproved);
        ASTRO.burnFrom(msg.sender, _amount);
        sASTRO.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        astroDebt = astroDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(ASTRO), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SASTRO) {
            sASTRO = IsASTRO(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SASTRO) {
            // 9
            sASTRO = IsASTRO(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(ASTRO.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns ASTRO valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(ASTRO)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return ASTRO.totalSupply() - astroDebt;
    }
}
