...
abstract contract AstronosAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(IAstronosAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IAstronosAuthority public authority;


    /* ========== Constructor ========== */

    constructor(IAstronosAuthority _authority) {
    ...
    function setAuthority(IAstronosAuthority _newAuthority) external onlyGovernor {
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gASTRO remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgASTRO) external returns (uint256);
  function redeemAll(address _user, bool _sendgASTRO) external returns (uint256);
...
interface IgASTRO is IERC20 {
  ...
  function migrate( address _staking, address _sASTRO ) external;
}

pragma solidity ^0.8.10;

abstract contract FrontEndRewarder is AstronosAccessControlled {
  ...
  IERC20 internal immutable astro; // reward token

  constructor(
    IAstronosAuthority _authority, 
    IERC20 _astro
  ) AstronosAccessControlled(_authority) {
    astro = _astro;
  }
  ...
  function getReward() external {
    ...
    astro.transfer(msg.sender, reward);
  }
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in ASTRO (false, default)
  ...
  struct Terms {
    ...
    uint64 maxDebt; // 9 decimal debt maximum in ASTRO
  }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  IgASTRO internal immutable gASTRO;
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _astro) {
    gASTRO = _gastro;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of ASTRO due to the user
  ...
  function addNote(
    ...
    notes[_user].push(
      Note({
        payout: gASTRO.balanceTo(_payout),
    ...
    // note that only the payout gets staked (front end rewards are in ASTRO)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgASTRO    send payout as gASTRO or sASTRO
   * @return payout_     sum of payout sent, in gASTRO
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgASTRO) public override returns (uint256 payout_) {
    ...
    if (_sendgASTRO) {
      gASTRO.transfer(_user, payout_); // send payout as gASTRO
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sASTRO
    }
  }
   ...
   * @param _sendgASTRO    send payout as gASTRO or sASTRO
   * @return             sum of payout sent, in gASTRO
   */ 
  function redeemAll(address _user, bool _sendgASTRO) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgASTRO);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gASTRO
...
/// @title Astronos Bond Depository V2

contract AstronosBondDepositoryV2 is IBondDepository, NoteKeeper {
/* ======== DEPENDENCIES ======== */
...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    ...
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _astro, _gastro, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _astro.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gASTRO due
  ...
  function deposit(
     ...
     * price = quote tokens : astro (i.e. 42069 DAI : ASTRO)
     *
     * 1e18 = ASTRO decimals (9) + price decimals (9)
     */
     ...
     *
     * this is either the number of ASTRO that the market can sell
    ...
    // purchased, and how much ASTRO has been sold
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // astro decimals (9) + price decimals (9)
       ...
       * is 10,000 ASTRO. max payout would be 1,000 ASTRO (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in ASTRO or quote), initial price / ASTRO (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = astro decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 ASTRO, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 ASTRO.
     */
    ...
    emit CreateMarket(id_, address(astro), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in ASTRO decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of ASTRO to be paid in ASTRO decimals
   *
   * @dev 1e18 = astro decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in ASTRO decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in ASTRO decimals
   */
