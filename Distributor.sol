...
///         ASTRO, as well as patches a small bug in the staking contract
...
contract Distributor is AstronosAccessControlled {
    ...
    /// The ASTRO Token
    IERC20 private immutable astro;
    /// The Astronos Treasury
    ITreasury private immutable treasury;
    /// The ASTRO Staking Contract
    ...
    function triggerRebase() external {
        unlockRebase = true;
        IStaking(staking).unstake(msg.sender, 0, true, true); // Give the caller the bounty astro.
    ...
    function nextRewardFor(address who) public view returns (uint256) {
        return (astro.balanceOf(who) * rewardRate) / DENOMINATOR;
    }
