...
contract sAstronos is IsASTRO, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgASTRO public gASTRO; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked ASTRO", "sASTRO", 9) ERC20Permit("Staked ASTRO") {
    ...
    function setgASTRO(address _gASTRO) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gASTRO) == address(0), "gASTRO:  gASTRO already set");
        require(_gASTRO != address(0), "gASTRO:  gASTRO is not a valid contract");
    ...
    /**
        @notice increases rASTRO supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sASTRO of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sASTRO
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sASTRO: insufficient balance");
    }
    ...
    // toG converts an sASTRO balance to gASTRO terms. gASTRO is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gASTRO.balanceTo(amount);
    }

    // fromG converts a gASTRO balance to sASTRO terms. sASTRO is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gASTRO.balanceFrom(amount);
    }

    // Staking contract holds excess sASTRO
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gASTRO.balanceFrom(IERC20(address(gASTRO)).totalSupply())).add(
